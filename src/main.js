...
function renderBrief(data){const summary=data.summary||{};$('#todayBrief').innerHTML=[['TODAY','今日任务',summary.today||0],['OVERDUE','已经逾期',summary.overdue||0,'alert'],['REMAIN','待办总数',summary.remaining||0],['COMPLETE','累计完成',summary.completed||0]].map(([code,label,value,kind])=>`<div class="brief-card ${kind||''}"><small>${code} // ${label}</small><b>${value}</b></div>`).join('')}
...
function renderSchedules(){const list=$('#scheduleList'),items=filteredSchedules();list.innerHTML='';if(!items.length){list.innerHTML='<div class="schedule-empty">◇<br>当前视图没有任务</div>';return}items.forEach(item=>{const row=document.createElement('div');row.className=`schedule-item ${item.priority||'normal'} ${item.completed?'done':''}`;const due=item.dueAt?new Date(item.dueAt):null,delta=due?due-Date.now():Infinity,dueClass=delta<0&&!item.completed?'overdue':delta<3600000&&!item.completed?'soon':'';row.innerHTML=`<button class="schedule-check">${item.completed?'✓':''}</button><p><b></b><small><em class="schedule-due ${dueClass}"></em>${item.repeat!=='none'?`<span class="schedule-repeat">${repeatNames[item.repeat]||item.repeat}</span>`:''}</small></p><div class="schedule-actions"><button class="schedule-delete" title="删除">×</button></div>`;row.querySelector('b').textContent=item.title;row.querySelector('.schedule-due').textContent=due?due.toLocaleString(locale(),{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}):'无截止时间';row.querySelector('.schedule-check').onclick=()=>updateSchedule(item.id,{completed:!item.completed});row.querySelector('.schedule-delete').onclick=()=>deleteSchedule(item.id);list.append(row)})}
function renderHudSchedules(){const active=scheduleItems.filter(x=>!x.completed).slice(0,5),box=$('#tasks');box.innerHTML='';if(!active.length){box.innerHTML='<div class="empty"><span>◇</span><b>暂无运行任务</b><small>用语音或文字下达指令</small></div>';return}active.forEach(item=>{const row=document.createElement('div');row.className='task';row.innerHTML='<i></i><div><b></b><small></small></div><span>○</span>';row.querySelector('b').textContent=item.title;row.querySelector('small').textContent=item.dueAt?new Date(item.dueAt).toLocaleString(locale(),{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}):'待办';box.append(row)})}
...
async function loadKnowledge(){try{const data=await(await fetch('/api/knowledge')).json(),list=$('#knowledgeList');$('#knowledgeCount').textContent=data.total||0;list.innerHTML='';if(!data.items?.length){list.innerHTML='<div class="knowledge-empty">◇<br>尚未添加本地资料</div>';return}data.items.forEach(item=>{const row=document.createElement('div');row.className='knowledge-doc';row.innerHTML='<i></i><p><b></b><small></small></p><button title="删除文档">×</button>';row.querySelector('i').textContent=item.type.toUpperCase();row.querySelector('b').textContent=item.name;row.querySelector('small').textContent=`${item.chunkCount} 个片段 · ${(item.size/1024).toFixed(1)} KB · ${new Date(item.createdAt).toLocaleDateString(locale())}`;row.querySelector('button').onclick=()=>deleteKnowledge(item.id);list.append(row)})}catch(error){showToast(error.message||'知识库读取失败')}}
