...
app.innerHTML = `
  ...
  <header class="topbar">
    ...
    <nav class="main-nav" aria-label="主菜单">
      ...
      <button id="taskCenterNav" title="任务与日程">▱<small>PLAN</small></button>
...
const toolDefinitions = [
  ...
  {type:'function',function:{name:'create_task',description:'创建一个待办任务',parameters:{type:'object',properties:{title:{type:'string'},notes:{type:'string'},priority:{type:'string',enum:['low','medium','high']},dueAt:{type:'string',description:'带时区的 ISO 时间'}},required:['title']}}},
  {type:'function',function:{name:'create_event',description:'创建一项日程',parameters:{type:'object',properties:{title:{type:'string'},notes:{type:'string'},startAt:{type:'string',description:'带时区的 ISO 时间'},endAt:{type:'string',description:'带时区的 ISO 时间'}},required:['title','startAt','endAt']}}},
  {type:'function',function:{name:'query_schedule',description:'查询指定本地日期的任务与日程',parameters:{type:'object',properties:{date:{type:'string',description:'YYYY-MM-DD'}},required:['date']}}}
];
...
async function executeTool(name, args) {
  ...
  if(name==='create_task'){const response=await fetch('/api/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(args)});const data=await response.json();if(!response.ok)return data.error;addTask(`${english?'Task':'任务'}：${data.task.title}`,english?'Pending':'待办');refreshTaskCenter();return english?'Task created':'任务已创建';}
  if(name==='create_event'){const response=await fetch('/api/events',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(args)});const data=await response.json();if(!response.ok)return data.error;addTask(`${english?'Event':'日程'}：${data.event.title}`);refreshTaskCenter();return data.conflicts?.length?(english?'Event created with a time conflict':'日程已创建，但存在时间冲突'):(english?'Event created':'日程已创建');}
  if(name==='query_schedule'){const [trsp,ersp]=await Promise.all([fetch(`/api/tasks?date=${encodeURIComponent(args.date)}`),fetch(`/api/events?date=${encodeURIComponent(args.date)}`)]);const tasks=(await trsp.json()).tasks||[],events=(await ersp.json()).events||[];if(!tasks.length&&!events.length)return english?'No tasks or events for that date':'该日期没有任务或日程';return [...tasks.map(item=>`${english?'Task':'任务'}: ${item.title}`),...events.map(item=>`${english?'Event':'日程'}: ${item.title} (${new Date(item.startAt).toLocaleTimeString(locale(),{hour:'2-digit',minute:'2-digit'})})`)].join('\n');}
