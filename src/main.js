...
function syncWorkspaceDetail(){document.querySelectorAll('[data-mode]').forEach(button=>button.classList.toggle('active',button.dataset.mode===currentWorkspaceMode));$('#workspaceMessageCount').textContent=workspaceMessages().length;$('#currentWorkspaceLabel').textContent=$('#workspaceName').value||tr('临时会话')}
async function loadWorkspaceList(query=''){try{const data=await(await fetch(`/api/workspaces?query=${encodeURIComponent(query)}`)).json(),list=$('#workspaceList');list.innerHTML='';if(!data.items?.length){list.innerHTML=`<div class="workspace-list-empty">◇<br>${tr('还没有已保存的工作区')}</div>`;return}data.items.forEach(item=>{const card=document.createElement('div');card.className=`workspace-card ${item.id===currentWorkspaceId?'active':''}`;card.innerHTML='<b></b><p></p><small></small><em></em><button>×</button>';card.querySelector('b').textContent=item.title;card.querySelector('p').textContent=item.preview||tr('暂无消息');card.querySelector('small').textContent=new Date(item.updatedAt).toLocaleString(locale());card.querySelector('em').textContent=getLanguage()==='en'?`${item.messageCount} msgs`:`${item.messageCount} 条`;card.querySelector('button').title=tr('删除');card.onclick=e=>{if(e.target.tagName!=='BUTTON')loadWorkspace(item.id)};card.querySelector('button').onclick=()=>deleteWorkspace(item.id);list.append(card)})}catch(error){showToast(error.message||tr('工作区读取失败'))}}
...
window.addEventListener('jarvis:languagechange',()=>{if(recognition)recognition.lang=getLanguage()==='en'?'en-US':'zh-CN';setConnected(connected);updateLanguageToggle();renderSettingsMemories();loadSchedules();syncWorkspaceDetail();if($('#workspaceModal').classList.contains('open'))loadWorkspaceList($('#workspaceSearch').value)});
